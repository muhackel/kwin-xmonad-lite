...
2. Kein Signal für Änderungen der `clientArea` (Panel). Proxys sind `windowAdded/Removed` eines Docks, dessen `frameGeometryChanged`/`outputChanged` und `virtualScreenGeometryChanged`, plus verzögerte Nachläufe (Muster Karousel `World.ts:34-40`, Krohnkite-Issue #35). **In Meilenstein 4 gemessen** (`docs/research.md` Abschnitt 3.3): ein Dock feuert `frameGeometryChanged` bei einer Panelhöhenänderung, und dort ist `clientArea` bereits neu. Nach einer **Ausgabenänderung** dagegen ist sie im Signal noch die alte und nach 500 ms teils noch ein Zwischenstand; erst nach 1500 ms stimmte sie. Deshalb zwei Nachläufe.
...
| Wo der Registry-GC sitzt (Meilenstein 4) | in `kwin/purge.ts` als reine Funktion auf dem Snapshot, nicht im Adapter. Der Adapter ruft nur `purgeFromSnapshot`; damit steht der ganze Ghost-Purge unter `node --test`. Eine leere Activity- oder Desktopliste gilt als misslungener Lesedurchgang und löscht nichts. |
