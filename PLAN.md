...
2. Kein Signal für Änderungen der `clientArea` (Panel). Proxys sind `windowAdded/Removed` eines Docks, dessen `frameGeometryChanged`/`outputChanged` und `virtualScreenGeometryChanged`, plus verzögerte Nachläufe (Muster Karousel `World.ts:34-40`, Krohnkite-Issue #35). **In Meilenstein 4 gemessen** (`docs/research.md` Abschnitt 3.3): ein Dock feuert `frameGeometryChanged` bei einer Panelhöhenänderung, und im entprellten Lauf danach ist `clientArea` bereits neu (der Moment des Signals selbst ist für diesen Fall nicht gemessen). Nach einer **Ausgabenänderung** dagegen ist sie im Signal noch die alte und nach 500 ms teils noch ein Zwischenstand; erst nach 1500 ms stimmte sie. Deshalb zwei Nachläufe.
