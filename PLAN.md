...
- **Flake-Outputs:**
  ...
  - `checks`: `package` (Typcheck, Tests und Bundle in der buildPhase), `lint` (Biome), `scripts` (alle Werkzeuge samt shellcheck) und `home-module` (baut ein `activationPackage` mit aktiviertem Modulzweig und prüft die von plasma-manager erzeugte `data.json` mit `jq` auf das Plugin-Flag, alle sechs Schlüssel und einen gesetzten `xml-*`-Shortcut); `nix flake check` grün über neun Prüfungen.
...
| 20c | Fokusziel zwischen Tastendruck und Lauf **geschlossen** | `aktivieren fehlgeschlagen für …`, genau eine Zeile, kein Wurf | **offen**, nicht reproduzierbar herstellbar |
