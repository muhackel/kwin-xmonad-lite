...
test("full gibt jedem Fenster die ganze Flaeche minus Aussenabstand", () => {
...
test("full ignoriert das Master-Verhaeltnis und den Innenabstand", () => {
	const a = full(SCREEN, 2, { ratio: 0.1, gapOuter: 0, gapInner: 0 });
	const b = full(SCREEN, 2, { ratio: 0.9, gapOuter: 0, gapInner: 64 });
	assert.deepEqual(a, b);
});

test("full erfuellt die Layouteigenschaften ueber Gitter und Fuzz", () => {
	for (const testCase of allCases()) {
		...
		for (let i = 0; i < rects.length; i++) {
			...
			assert.ok(equals(r, inner), `Zelle ${i} ist nicht die ganze Flaeche: ${label(testCase)}`);
		}
	}
});

test("full liefert eigenstaendige Objekte je Zelle", () => {
