...
// Wird mit jedem Meilenstein um die tatsächlich benutzten Teile erweitert.
//
// tsconfig hat "types": [] und "lib": ["ES2016"] -- was hier fehlt, existiert
// für den Typprüfer nicht. Alle Angaben sind an KWin 6.7.4 gemessen --
 ...
 *
 * `Signal0` heißt hier **gemessen ohne Argumente**. Wo Argumente kommen, hat
 * das Signal einen eigenen Typ -- auch dann, wenn der Adapter sie nicht liest:
 * eine falsche Argumentzahl in der Deklaration fällt sonst niemandem auf.
 ...
 * Während der Folge ist `workspace.currentDesktop` noch nicht umgestellt;
 ...
 * nicht den Wechsel -- genau der Auslöser, den der Registry-GC braucht.
 */
...
interface KwinOutput {
	/** DRM-Connector wie `DP-1`, portstabil. Schlüsselbestandteil. */
...
interface KwinVirtualDesktop {
	...
	readonly id: string;
	/** Nur für lesbare Journalausgaben. */
...
// und Indexzugriff -- der Typprüfer fängt einen `.map`-Griff damit ab,
// bevor er zur Laufzeit im Journal auffällt.

interface KwinWindow {
	/** QUuid-Objekt, kein String. Nur über `String(...)` verwendbar. */
	...
	/** Leere Liste heißt "alle Desktops" (an einem Dock gemessen). */
	readonly desktops: ArrayLike<KwinVirtualDesktop>;
	/** Leere Liste heißt "alle Activities". */
	readonly activities: ArrayLike<string>;
	readonly onAllDesktops: boolean;

	/** Schreiben heißt `moveResize` -- ohne Klemmung, ohne Maximiert-Prüfung. */
	frameGeometry: QRect;
	...
	/** Läuft gerade ein interaktives Verschieben bzw. Größenändern. */
...
declare const workspace: {
	...
	/** Alle virtuellen Desktops. Ist-Menge für `purgeSurfaces`. */
	readonly desktops: ArrayLike<KwinVirtualDesktop>;
	...
	/** Das Desktop-Argument ist in 6.7.4 wirkungslos, gehört aber zur Signatur. */
	...
	readonly desktopsChanged: Signal0;
	/** Einziges Signal über die Menge der Ausgaben; kommt in der Folge zuletzt. */
 ...
 * Verhalten -- `currentDesktopForScreen` mit Fallback deckt beide Fälle ab --,
 * aber ohne die Journalzeile ist ein späterer Auszug nicht deutbar.
 ...
 * liefert nichts -- Namen müssen bekannt sein.
 */
declare const KWin: {
...
/**
 * Der einzige Weg zu einer Verzögerung: `setTimeout` gibt es nicht.
